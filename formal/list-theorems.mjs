...
let sorry = 0; const theorems = [];
for (const f of files) {
  const lines = readFileSync(DIR + 'ProveML/' + f, 'utf8').split('\n');
  lines.forEach((l, i) => {
    if (/\bsorry\b/.test(l)) sorry++;
    const m = l.match(/^theorem\s+(\S+)/);
    if (m) { let doc = ''; for (let j = i - 1; j >= 0 && lines[j].trim(); j--) { const d = lines[j].match(/^\/--\s*(.*?)\s*-\/\s*$/) || lines[j].match(/^\/--\s*(.*)$/); if (d) { doc = d[1]; break; } } theorems.push(`${f.replace('.lean', '')}.${m[1]}${doc ? ': ' + doc : ''}`); }
  });
}
