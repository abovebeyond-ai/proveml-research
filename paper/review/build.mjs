...
const snapshots = {
  ...
  ...(existsSync('report/sources/raw/run-dates.txt') ? { dates: read('run-dates.txt') } : {}),
  ...(existsSync('report/sources/raw/repo-history.txt') ? { history: read('repo-history.txt') } : {}),
...
const bound = [
  ...
  { anchor: 'Entity references', id: 'entities', kind: 'para', marks: [
      ...
      ['which entity a fact binds to follows from the markup by fixed rules', 'which entity a fact binds to %[formal:defs.binding]{follows from the markup by fixed rules}'],
      ['a fact outside binds to the last simple-form entity at the current depth', '%[formal:defs.carry]{a fact outside binds to the last simple-form entity at the current depth}'],
      ['closing a scope restores the context that was in force when it opened', '%[formal:theorems.scopeRestores]{closing a scope restores the context that was in force when it opened}'],
    ], evidence: [
      q('formal:theorems.scopeRestores', 'closing a scope restores the context that was in force when it opened', 'theorems', 'Theorems.scope_restores: Closing a scope restores the entity that was in force when it opened, whatever the body did.', 'theorem list, Theorems.lean', 'Proven for a scope whose body opens and closes no scope of its own (a flat body); nested scopes follow by applying it inside out. Fair reading?'),
  ...
  { anchor: 'Verification resolves each construct against the store', id: 'verification', marks: [
      ['string equality for entities and facts, typed comparison for thresholds', '%[techreport:sections.comparisonRule]{string equality for entities and facts, typed comparison for thresholds}'],
      ['`NOT` of an unverifiable condition is still unverifiable', '`NOT` %[formal:theorems.notUu]{of an unverifiable condition is still unverifiable}'],
      ['it can equally audit text it did not generate', '%[verifier:inputs.textOnly]{it can equally audit text it did not generate}'],
      ['the share of standalone numbers that sit inside a fact claim', '%[verifier:coverage.section2]{the share of standalone numbers that sit inside a fact claim}'],
      ['in strict mode treats each number outside one as a finding', '%[verifier:strict.finding]{in strict mode treats each number outside one as a finding}'],
    ], evidence: [
      q('verifier:strict.finding', 'in strict mode treats each number outside one as a finding', 'verifier', 'strict mode: the same text has 0 finding(s) without strict and 1 with it', 'strict mode line', 'The prose number 2 becomes an error under options.strict and is none without it. Fair reading?'),
      q('verifier:coverage.section2', 'the share of standalone numbers that sit inside a fact claim', 'verifier', 'marked 1 (numbers inside a fact claim), unmarked 1 (numbers in prose), rate 0.5', 'coverage line', 'Numbers only, by design: a sentence with no number and a name in prose are outside it, as the limitations say. Fair reading?'),
      q('verifier:inputs.textOnly', 'it can equally audit text it did not generate', 'verifier', 'inputs: verifyProveml(markdown, store, options) takes 3 arguments, none of them a model or a generation; every claim above was written by hand, not generated', 'inputs line', 'The whole run is hand-written markup judged by the verifier; this paper\'s own review is the same layer over human-written text. Fair reading?'),
      q('formal:theorems.notUu', 'of an unverifiable condition is still unverifiable', 'theorems', 'Kleene.not_uu: Negating the unresolved leaves it unresolved: NOT of an unverifiable condition is still unverifiable.', 'theorem list, Kleene.lean', 'Proven by definition of the connective; the verifier run shows it: ?[p: NOT IS_HUGE_REVENUE] with the name unregistered is unverifiable. Fair reading?'),
      q('techreport:sections.comparisonRule', 'string equality for entities and facts, typed comparison for thresholds', 'techreport', 'Direct fact references use exact string equality against the store\'s canonical representation', 'section Fact Store', 'The report states the fact rule here and the threshold rule (typed comparison against the registry\'s predicate) in section Threshold Registry; the verifier run shows all three: a name mismatch on a string, a value mismatch on a string, a numeric bound that verifies. Fair reading?'),
    ] },
  { anchor: 'The store is the single point of trust', id: 'store', marks: [
      ['(deployments can bind a snapshot identifier to each result)', '(%[verifier:store.snapshot]{deployments can bind a snapshot identifier to each result})'],
      ['a cross-entity difference is materialized as an addressable fact', '%[verifier:store.arithmetic]{a cross-entity difference is materialized as an addressable fact}'],
      ['tells the renderer to show a verified', '%[verifier:store.display]{tells the renderer to show a verified}'],
      ['with the canonical value kept on the element and in the audit view', '%[verifier:store.canonical]{with the canonical value kept on the element and in the audit view}'],
    ], evidence: [
      q('verifier:store.snapshot', 'deployments can bind a snapshot identifier to each result', 'verifier', 'snapshot: the deployment passes snapshot "edgar-fy2025-2026-09-12" and the result carries snapshot "edgar-fy2025-2026-09-12" beside its 2/2 verified', 'snapshot line', 'The identifier is an option of the verifier and is echoed on the result. Fair reading?'),
      q('verifier:store.arithmetic', 'a cross-entity difference is materialized as an addressable fact', 'verifier', 'arithmetic in the data layer: the difference is stored as region:EU._salesDiff = 12000 USD; the fact %[_salesDiff]{12000 USD} is verified and the judgment ?[g: SALES_GAP_MATERIAL] against it is verified', 'arithmetic line', 'The difference is a stored value with a path, and a registered predicate bounds it; the verifier computes nothing. Fair reading?'),
      q('verifier:store.display', 'tells the renderer to show a verified', 'verifier', 'canonical value: claim %[revenue]{391035000000 USD} against company:aapl.revenue = 391035000000: verified, shown as $391.0 billion', 'canonical value line', 'The display rule currency:USD:1 on the store turns the verified canonical value into $391.0 billion. Fair reading?'),
      q('verifier:store.canonical', 'with the canonical value kept on the element and in the audit view', 'verifier', 'render: the reader sees $391.0 billion, the audit keeps 391035000000 USD', 'render line', 'The rendered HTML carries both. Fair reading?'),
    ] },
  { anchor: 'Inference references', id: 'inferences', marks: [
      ['names a threshold from a registry defined outside generation', 'names a threshold from %[verifier:infer.outside]{a registry defined outside generation}'],
      ...
      q('verifier:infer.outside', 'a registry defined outside generation', 'verifier', 'registry from outside: the deployment passes {IS_HUGE_REVENUE: revenue >= 100000000000} to the verifier and the same judgment is verified; without it, unverifiable (line above)', 'registry line', 'The registry is an argument the deployment hands to the verifier (options.thresholds in the package); the text has no construct that can add a name to it. Fair reading?'),
  ...
  { anchor: 'This paper is deliberately compact', id: 'intro-compact', marks: [
      ...
      ["the technical report's July 2026 study of smaller models", "the technical report's %[techreport:sections.julyStudy]{July 2026 study} of smaller models"],
      ['its finance runs date from March 2026, its education runs from the turn of July and August', 'its finance runs date from %[runs:dates.financeSmall]{March 2026}, its education runs from %[runs:dates.educationSmall]{the turn of July and August}'],
    ], evidence: [
      ...
      q('runs:dates.financeSmall', 'March 2026', 'dates', 'finance runs of the small-model study: 12 files, 2026-03-30 to 2026-03-30', 'run-dates.mjs', 'All twelve on 30 March 2026.'),
      q('runs:dates.educationSmall', 'the turn of July and August', 'dates', 'education runs of the small-model study: 24 files, 2026-07-31 to 2026-08-01', 'run-dates.mjs', '31 July to 1 August 2026. Fair reading?'),
      q('techreport:sections.julyStudy', 'July 2026 study', 'techreport', 'July 2026 study of four small local models', 'introduction', 'The report\'s own label; the parenthesis in the paper gives the run dates (finance March, education the turn of July and August 2026), and the current study ran on 29 August. Fair reading?'),
    ] },
  ...
  { anchor: 'Two things decide whether ProveML fits a use', id: 'intro-limits', marks: [
      ['a judgment the deployment never named cannot be made checkable by the model', '%[formal:theorems.deploymentNamed]{a judgment the deployment never named cannot be made checkable by the model}'],
      ...
      q('formal:theorems.deploymentNamed', 'a judgment the deployment never named cannot be made checkable by the model', 'theorems', 'Theorems.evalAtom_tt_registered: An atom that holds names a registered threshold.', 'theorem list, Theorems.lean', 'With evalCond_empty and evalCond_mono: an unregistered name leaves the judgment unresolved, never verified and never failed; the package\'s adversarial tests (attacks.test.js) exercise the same. Fair reading?'),
  ...
  { anchor: 'Organizations deploy LLMs widely', id: 'abstract-mechanism', marks: [
      ...
      ['coverage, the share of a text\'s numbers that sit inside a fact claim rather than in prose', 'coverage, %[verifier:coverage.definition]{the share of a text\'s numbers that sit inside a fact claim rather than in prose}'],
      ['two guarantees are machine-checked: a verified fact equals the store\'s value, and a judgment is verified only by a threshold the registry holds; what the verifier cannot resolve, it neither verifies nor fails', 'two guarantees are %[formal:theorems.checked]{machine-checked}: %[formal:theorems.factSound]{a verified fact equals the store\'s value}, and %[formal:theorems.registered]{a judgment is verified only by a threshold the registry holds}; %[formal:theorems.unresolved]{what the verifier cannot resolve, it neither verifies nor fails}'],
    ], evidence: [
      q('verifier:coverage.definition', "the share of a text's numbers that sit inside a fact claim rather than in prose", 'verifier', 'marked 1 (numbers inside a fact claim), unmarked 1 (numbers in prose), rate 0.5', 'coverage line', 'The verifier run on a sentence with one fact and one prose number; years, list markers and code are left out of the count. Is the abstract\'s definition a fair reading of what it computes?'),
      ...
      q('formal:theorems.factSound', "a verified fact equals the store's value", 'theorems', 'Theorems.verify_fact_sound: Lifted to a document: every fact the verifier calls verified is the store\'s own surface value.', 'theorem list, Theorems.lean', 'With verify_entity_sound for entity names. Is the abstract\'s phrase a fair reading of what it proves?'),
      q('formal:theorems.registered', 'a judgment is verified only by a threshold the registry holds', 'theorems', 'Theorems.evalAtom_tt_registered: An atom that holds names a registered threshold.', 'theorem list, Theorems.lean', 'With evalAtom_tt_holds: the bound path holds a value, the unit matches, and the operator holds of it (for every operator but is_null, Appendix B). Fair reading?'),
      q('formal:theorems.unresolved', 'what the verifier cannot resolve, it neither verifies nor fails', 'theorems', 'Theorems.evalCond_empty: In particular, against the empty registry nothing holds and nothing fails: every judgment is unresolved.', 'theorem list, Theorems.lean', 'With evalCond_mono: an unresolved judgment can later be decided by a fuller registry, a decided one never flips. A missing value or unit leaves an atom unresolved by definition (Appendix B), except under is_null, which the paper names as the gap to close. Fair reading?'),
  ...
  { anchor: 'three current models of August 2026', id: 'abstract-results', marks: [
      ...
      ['the commonest failure is the word the question invited', '%[study:judgment.commonest]{the commonest failure is the word the question invited}'],
    ], evidence: [
      ...
      q('study:judgment.commonest', 'the commonest failure is the word the question invited', 'judgment', '13 of 33 (contra_unreliable 4, bound_strong_74 3, bound_sample_5 3, bound_grey_29 2, contra_strong_student 1), the largest single class; the other 20 spread over 6 questions', 'judgment-summary.mjs, invited-word line', 'Counted by the threshold\'s own word appearing in the question (strong, small, grey). Is "commonest" a fair reading of 13 of 33 against a rest spread over six questions, the largest of them 6?'),
  ...
  { anchor: 'Three models current in August 2026', id: 'setup', marks: [
      ['Claude Opus 5, released 24 July 2026', '@[model:opus5]{Claude Opus 5}, released %[model:opus5.released]{24 July 2026}'],
      ...
      ['it was withdrawn, replaced, and every educational result re-measured on the replacement', '%[repo:history.withdrawn]{it was withdrawn}, replaced, and %[repo:history.remeasured]{every educational result re-measured on the replacement}'],], evidence: [
      q('repo:history.withdrawn', 'it was withdrawn', 'history', 'Runs against the withdrawn education dataset: kept locally for reference,', 'the repository\'s .gitignore', 'The withdrawn dataset\'s runs are kept out of the repository by rule; the artefacts of 31 July (cbed76e) preceded the replacement. Fair reading?'),
      q('repo:history.remeasured', 'every educational result re-measured on the replacement', 'history', '2026-08-01 94e91f6 Onderwijsresultaten opnieuw gemeten op de gegenereerde dataset', 'commit of 1 August 2026', 'Dutch: educational results re-measured on the generated dataset; the commit body lists 39 runs redone. Fair reading?'),
...
const inferSubject = (pidv, text) => {
  ...
  for (let c of list) {
    ...
    if (/^\(?(?:Section|Table|Figure|Appendix)\s*[\dA-Z]+\b/.test(c.span.trim())) continue;   // a sentence that starts on a cross-reference is navigation ("Section 3 shows why that is needed"); the reader follows it
