...
const store = {
  ...
  'verifier:entity.nameCheck': 'checks the display name against the store', 'verifier:fact.equality': 'exact string equality', 'verifier:infer.noInvent': 'the model cannot invent a comparison value, a bound, or a direction', 'verifier:infer.outside': 'a registry defined outside generation', 'verifier:store.snapshot': 'deployments can bind a snapshot identifier to each result', 'techreport:sections.comparisonRule': 'string equality for entities and facts, typed comparison for thresholds', 'formal:theorems.notUu': 'of an unverifiable condition is still unverifiable', 'verifier:inputs.textOnly': 'it can equally audit text it did not generate', 'verifier:coverage.section2': 'the share of standalone numbers that sit inside a fact claim', 'verifier:strict.finding': 'in strict mode treats each number outside one as a finding', 'verifier:store.arithmetic': 'a cross-entity difference is materialized as an addressable fact', 'verifier:store.display': 'tells the renderer to show a verified', 'verifier:store.canonical': 'with the canonical value kept on the element and in the audit view', 'verifier:infer.unregistered': 'an unregistered name is an error rather than a claim', 'verifier:fact.outcomes': 'verified, mismatch, unverifiable (no such field), no context', 'formal:defs.binding': 'follows from the markup by fixed rules', 'formal:defs.carry': 'a fact outside binds to the last simple-form entity at the current depth', 'formal:theorems.scopeRestores': 'closing a scope restores the context that was in force when it opened',
