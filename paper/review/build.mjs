...
const store = {
  ...
  'bench:finance.name': 'the finance benchmark', 'bench:finance.prompts': 10, 'bench:finance.entities': 2, 'bench:finance.fields': 11, 'bench:finance.origin': 'real SEC EDGAR FY2025 filings', 'study:frontier.runsPer': 'Three runs per model and benchmark', 'study:frontier.correctionBudget': 'one correction pass', 'study:frontier.noEmpty': 'no run on either benchmark produced one',
...
const bound = [
  ...
  { anchor: 'This appendix states what the verifier computes', id: 'formal-intro', marks: [
      ['mechanised in Lean 4 in the artifact repository (`formal/`, no dependencies beyond Lean)', 'mechanised in %[formal:theorems.lean4]{Lean 4} in the artifact repository (`formal/`, %[formal:theorems.noDeps]{no dependencies beyond Lean})'],
      ['11 of 11 agree', '%[formal:vectors.agree]{11} of %[formal:vectors.total]{11} agree'],
    ], evidence: [
      q('formal:theorems.lean4', 'Lean 4', 'theorems', 'toolchain: leanprover/lean4:v4.33.1; packages required in lakefile.toml: none', 'toolchain line', 'The toolchain pinned in formal/lean-toolchain. Fair reading?'),
      q('formal:theorems.noDeps', 'no dependencies beyond Lean', 'theorems', 'toolchain: leanprover/lean4:v4.33.1; packages required in lakefile.toml: none', 'toolchain line', 'No [[require]] in the lakefile: the model builds on the Lean core alone, no Mathlib. Fair reading?'),
  ...
  { anchor: 'Three models current in August 2026', id: 'setup', marks: [
      ...
      ['Three runs per model and benchmark, one correction pass', '%[study:frontier.runsPer]{Three runs per model and benchmark}, %[study:frontier.correctionBudget]{one correction pass}'],
      ['no run on either benchmark produced one', '%[study:frontier.noEmpty]{no run on either benchmark produced one}'],], evidence: [
      ...
      q('study:frontier.noEmpty', 'no run on either benchmark produced one', 'summary', 'empty answers scored 0% (education harness): 0; empty answers dropped (finance harness writes null): 0', 'frontier-summary.mjs, design line', 'Both harness behaviours counted over the first study; the second study (frontier2-summary.txt) prints the same zeros. The difference in behaviour is in the two harness scripts (test-convergence.js scores 0%, test-convergence-finance.js writes null). Fair reading?'),
