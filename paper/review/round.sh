...
# Compared on the bound readings (marks and citations), not on the flag pass's spans: a shorter sentence
# has fewer spans without having lost a mark (round 7 of 2026-09-12 was refused on that until this changed).
readings_now=$(echo "$verified" | grep -o 'bound [0-9]*' | cut -d' ' -f2)
readings_prev=$(grep 'push [0-9]' report/rounds.txt 2>/dev/null | tail -1 | grep -o 'bound [0-9]*' | cut -d' ' -f2 || true)
if [ -n "${readings_prev:-}" ] && [ -n "${readings_now:-}" ] && [ "$readings_now" -lt "$readings_prev" ] && [ "${ALLOW_FEWER:-0}" != "1" ]; then
  echo "refused: bound readings went from $readings_prev to $readings_now; an edit lost a mark (ALLOW_FEWER=1 to push anyway)"; exit 1
fi
